...
class StudioApi(
  ...
  suspend fun listLessons(): List<LessonSummaryDto> = withContext(Dispatchers.IO) { get("v1/lessons") }
  suspend fun getLesson(id: String): LessonDetailDto = withContext(Dispatchers.IO) { get("v1/lessons/$id") }
  suspend fun createSession(body: CreateSessionRequest): SessionResponse = withContext(Dispatchers.IO) { post("v1/sessions", body) }
  suspend fun submit(
      sessionId: String,
      body: SubmitStepRequest,
  ): FeedbackResponse = withContext(Dispatchers.IO) { post("v1/sessions/$sessionId/submit", body) }
  suspend fun help(): List<HelpArticleDto> = withContext(Dispatchers.IO) { get("v1/help") }
  suspend fun sendCrash(ing: ClientCrashIngest): ClientCrashAck = withContext(Dispatchers.IO) { post("v1/client-logs", ing) }
